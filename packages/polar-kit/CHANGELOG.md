...
- **README:** Added key features section for better readability
...
### Added
- Initial release of @makeco/polar-kit CLI
...
- **BREAKING:** Commands restructured to use subcommand pattern:
  - `clear-db-plans` -> `polar-kit db purge`
  - `list-products` -> `polar-kit list products`
  - `list-prices` -> `polar-kit list prices`
